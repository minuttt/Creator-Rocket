...
async def analyze_creator(username: str, platform: str = "youtube", db: Session = Depends(get_db)):
    ...
    if creator:
        ...
        # AUTO-REFRESH: fetch fresh data if less than 2 snapshots or latest is older than 10 mins
        needs_update = False
        if len(snapshots) < 2:
            needs_update = True
        elif snapshots:
        ...
        if not stats:
            raise HTTPException(
                status_code=404,
                detail=f"Channel '{username}' not found. Check that your YOUTUBE_API_KEY is set correctly."
            )
